...
?>
<div class="bx-auth-reg" xmlns="http://www.w3.org/1999/html">
...
<form method="post" action="<?=POST_FORM_ACTION_URI?>" name="regform-<?=$arParams['CODE_TAB']?>" class="modal__form form" enctype="multipart/form-data" data-tab-id="toggle-reg" data-tab-block="<?=$arParams['CODE_TAB']?>">
<?
if($arResult["BACKURL"] <> ''):
?>
	<input type="hidden" name="backurl" value="<?=$arResult["BACKURL"]?>" />
		...
		case "PASSWORD":
            ...
            <div class="form__item">
                <label class="form__label">
                    <input placeholder="<?=GetMessage("REGISTER_FIELD_".$FIELD)?>" class="form__input" size="30" type="password" name="REGISTER[<?=$FIELD?>]" value="<?=$arResult["VALUES"][$FIELD]?>" autocomplete="off" />
                </label>
            </div>
<?
			break;
    ...
    <?endif;
			?>
    <div class="form__item"  <?	if ($FIELD == "LOGIN"):?> style="display: none" <? endif;?>>
        <label class="form__label">
            <input size="30" type="text" placeholder="<?=GetMessage("REGISTER_FIELD_".$FIELD)?>" name="REGISTER[<?=$FIELD?>]"
                <? if ($FIELD == "LOGIN"):?>
                   value="111111" class="form__input" />
            <? else :?>
                   value="<?=$arResult["VALUES"][$FIELD]?>" class="form__input" />
            <? endif?>
            <?
				if ($FIELD == "PERSONAL_BIRTHDAY")
...
?>
    ...
    <button type="submit" class="form__submit-btn button"><?=GetMessage("AUTH_REGISTER")?></button>
