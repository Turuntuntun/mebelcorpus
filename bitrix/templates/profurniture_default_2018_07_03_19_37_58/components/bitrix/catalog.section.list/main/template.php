		...
		<ul class="row list-unstyled">
			...
			foreach ($arResult['SECTIONS'] as $key => $arSection):
				...
				if ($arSection["DEPTH_LEVEL"] == $arResult['SECTION']['DEPTH_LEVEL'] + 1): ?>
                    ...
                    ?>
					<li class="section col-xs-12 col-md-6 col-md-4 col-lg-3" id="<?=$this->GetEditAreaId($arSection['ID']);?>">
                        <?php
				...
				<?php else: ?>
                    ...
                    if ($index2 > $arParams['SHOW_COUNT_LVL2']) {
						if ($arParams['SHOW_SHOW_MORE'] == 'Y' && $moreShowed === false && $index2 != 1 && $previousLevelOne > 0): ?>
							<li><a class="mainsections__show-more" href="<?=$arResult['SECTIONS'][$previousLevelOne]['SECTION_PAGE_URL']?>" title=""><?=Loc::getMessage('BUTTON.SHOW_MORE')?><?php if ($arParams['COUNT_ELEMENTS'] && intval($arResult['SECTIONS'][$previousLevelOne]['ELEMENT_CNT']) > 0): ?> (<?=$arResult['SECTIONS'][$previousLevelOne]['ELEMENT_CNT']?>)<?php endif;?>...</a></li>
						<?php
						$moreShowed = true;
						endif;
						continue;
