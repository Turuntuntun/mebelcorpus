...
if ($this->__component->getName() == 'bitrix:catalog.product.subscribe.list') {
	foreach ($arResult['ITEMS'] as $key1 => $arItem)
	{
		if (!empty($arParams['LIST_SUBSCRIPTIONS'][$arItem['ID']]))
			continue;
		...
		{
			if (empty($arParams['LIST_SUBSCRIPTIONS'][$arOffer['ID']]))
				continue;

			$arResult['ITEMS'][$arOffer['ID']] = $arOffer;
			$arResult['ITEMS'][$arOffer['ID']]['FIRST_PIC'] = $arItem['FIRST_PIC'];
			$arResult['ITEMS'][$arOffer['ID']]['DETAIL_PAGE_URL'] = $arItem['DETAIL_PAGE_URL'];
		}
...
// Передает данные в component_epilog
if (is_array($arResult['ITEMS']) && count($arResult['ITEMS']) > 0) {
	foreach ($arResult['ITEMS'] as $arItem) {
		$arResult["PRO_SHOW_PRICE"][$arItem["ID"]] = $arItem["ID"];
	}
}
$cp = $this->__component;
if (is_object($cp)) {
	$cp->SetResultCacheKeys(array("PRO_SHOW_PRICE"));
}