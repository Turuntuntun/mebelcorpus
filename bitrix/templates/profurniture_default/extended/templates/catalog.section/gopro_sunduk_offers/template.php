...
if (is_array($arResult['ITEMS']) && count($arResult['ITEMS']) > 0) {
	...
	if (file_exists($path = rsGoProGetTemplatePathPart(EXTENDED_PATH_TEMPLATES.'/catalog.section/gopro_sunduk/template.start.php', $getTemplatePathPartParams))) {
		include($path);
	}

	$templateExtFolder = EXTENDED_PATH_TEMPLATES.'/catalog.section/gopro_sunduk/';
	...
	// ajaxpages && sorterchange -> start
	if (file_exists($path = rsGoProGetTemplatePathPart(EXTENDED_PATH_TEMPLATES.'/catalog.section/gopro_sunduk/template.finish.php', $getTemplatePathPartParams))) {
		include($path);
	}
