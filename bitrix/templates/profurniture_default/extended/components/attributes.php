...
$svgArrowDown = '<svg class="c-attributes__arrow svg-icon"><use xlink:href="#svg-arrow-linear-down"></use></svg>';
...
if ($arParams['SHOW_SIZE_TABLE'] == 'Y')
{
    $iPropsSizeTablesCount = count(
        array_intersect_key(
            array_flip($arParams['PROPS_ATTRIBUTES_SIZE']),
            $arItem['OFFERS_EXT']['PROPERTIES']
        )
    );
}
    ...
    <!-- prop --><div class="c-attributes__prop js-attributes__prop js-attributes__code__<?=$propCode?> close<?php if ($isPic): ?> is-pic js-pic<?php endif;?>" data-code="<?=$propCode?>">
        ...
        foreach ($arProperty as $value => $arValue):
            ...
            ?><span class="c-attributes__name">
                <?=$arItem['OFFERS_EXT']['PROPS'][$propCode]['NAME']?>:
                <?php
                if ($arParams['SHOW_SIZE_TABLE'] == 'Y' && !empty($arResult['SIZE_TABLE']) && $iPropsSizeTablesCount == 1)
                {
                    if (in_array($propCode, $arParams['PROPS_ATTRIBUTES_SIZE']))
                    {
                        ?>
                        <a class="c-attributes__size-table c-size-table" href="#size-table" data-fancybox="size-table">
                            <svg class="svg-icon"><use xlink:href="#svg-ruler"></use></svg><span class="c-size-table__title"><?=Loc::getMessage('RS_GOPRO_EXT_COMPONENTS_ATTRIBUTES_SIZE_TABLE')?></span>
                        </a>
                        <?php
                    }
                }
...
if ($arParams['SHOW_SIZE_TABLE'] == 'Y' && !empty($arResult['SIZE_TABLE']) && $iPropsSizeTablesCount > 1)
{
    ?>
    <div>
        <a class="c-size-table" href="#size-table" data-fancybox="size-table">
            <svg class="svg-icon"><use xlink:href="#svg-ruler"></use></svg><span class="c-size-table__title"><?=Loc::getMessage('RS_GOPRO_EXT_COMPONENTS_ATTRIBUTES_SIZE_TABLE')?></span>
        </a>
    </div>
    <?php
...
{
    ?>
    <div id="size-table" class="restart-buffer" style="display:none"><?=$arResult['SIZE_TABLE']['PREVIEW_TEXT']?></div>
    <?php
}
