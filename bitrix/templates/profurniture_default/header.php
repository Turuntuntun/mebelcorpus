...
$obFile3 = new File(Application::getDocumentRoot().SITE_TEMPLATE_PATH.'/description.php');
if ($obFile3->isExists())
{
	include(Application::getDocumentRoot().SITE_TEMPLATE_PATH.'/description.php');    
}
...
$color = $tuning->getOptionValue('COLOR');
$mainMenuType = $tuning->getOptionValue('MAIN_MENU_TYPE')?:'horizontal1';
$headerType = $tuning->getOptionValue('HEADER_TYPE')?:'type1';
$useHeaderFly = $tuning->getOptionValue('SWITCH_USE_HEADER_FLY')?:'Y';
// safework
...
$asset->addCss(SITE_TEMPLATE_PATH.'/assets/lib/node-waves/waves.css');
...
$asset->addJs(SITE_TEMPLATE_PATH.'/assets/lib/node-waves/waves.js');
...
$asset->addJs(SITE_TEMPLATE_PATH.'/assets/js/animations.js');
...
// add fonts
include Application::getDocumentRoot().SITE_TEMPLATE_PATH.'/extended/font.php';


// add module version
if (!empty($arModuleVersion['VERSION']))
{
	$asset->addString('<meta property="gopro:module:version" content="'.$arModuleVersion['VERSION'].'">');
}
// add template version
if (!empty($arTemplate['GOPRO_VERSION']))
{
	$asset->addString('<meta property="gopro:template:version" content="'.$arTemplate['GOPRO_VERSION'].'">');
}
if (!empty($arTemplate['GOPRO_DATE_VERSION']))
{
	$asset->addString('<meta property="gopro:template:date_version" content="'.$arTemplate['GOPRO_DATE_VERSION'].'">');
}
...
<body class="<?
	...
	<div id="location_confirm_bg"></div>
	...
	<div class="body" itemscope itemtype="http://schema.org/WebPage"><!-- body -->
		...
		<?php if ($useHeaderFly == 'Y' || $headerType == 'type2'): ?><?
			...
			if ( SITE_ID == 'sm' )
			{
				require_once (EXTENDED_PATH_HEADERS.'/fly_sunduk.php');
			}
			else
			{
				require_once (EXTENDED_PATH_HEADERS.'/fly.php');
			}
		...
		if ( SITE_ID == 'sm' )
		{
			require_once (EXTENDED_PATH_HEADERS.'/type_sunduk.php');
		}
		else
		{
			require_once (EXTENDED_PATH_HEADERS.'/'.$headerType.'.php');
		}
		
		
		if ( SITE_ID == 'sm' )
		{
			require_once (EXTENDED_PATH_HEADERS.'/menu_horizontal_and_vertical.php');
		}
		else
		{
			require_once (EXTENDED_PATH_HEADERS.'/menu_'.$mainMenuType.'.php');
		}
		?>
