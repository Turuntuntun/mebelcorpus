...
?>
		...
		<?php if ($arParams['USE_FILTER'] == 'Y' && SITE_ID != 'sm'): ?>
			<?$APPLICATION->IncludeComponent(
...
?>
		...
		<?
		if ( SITE_ID == 'sm' && $APPLICATION->GetCurDir() != '/catalog/sale/' )
		{
			?>
				<?$APPLICATION->IncludeComponent(
					'bitrix:catalog.smart.filter',
...
if ( SITE_ID == 'sm' && $APPLICATION->GetCurDir() == '/catalog/sale/' )
{
	$APPLICATION->IncludeComponent(
		...
		array(
			...
			"ALFA_SORT_BY_DEFAULT" => "PROPERTY_DISCOUNT_PERCENT_desc",
...
<?
$sectionTemplate = 'gopro';
if ( SITE_ID == 'sm' )
{
	$sectionTemplate = 'gopro_sunduk';
	
	if ( $APPLICATION->GetCurDir() == '/catalog/sale/' )
	{
		...
		{
			$currentPercent = $arItem['PROPERTY_DISCOUNT_PERCENT_VALUE'];
			
			$arPrice = CCatalogProduct::GetOptimalPrice( $arItem['ID'] );
			$actualPercent = $arPrice['RESULT_PRICE']['PERCENT'];
			
			$dbOffer = CIBlockElement::GetList( Array(), Array( 'IBLOCK_ID' => 39, 'PROPERTY_CML2_LINK' => $arItem['ID'] ), false, Array( 'nTopCount' => 1 ), Array( 'ID' ) );
			...
			{
				CIBlockElement::SetPropertyValuesEx( $arItem['ID'], 29, Array( 'DISCOUNT_PERCENT' => $actualPercent ) );
			}
		}
