...
if ( CModule::IncludeModule( 'iblock' ) )
{
	...
	$dbSet = CIBlockElement::GetList( $arSort, $arFilter, false, $arNav, $arSelect );
	if ( $arSet = $dbSet->Fetch() )
	{
		$dbPropValues = CIBlockElement::GetProperty( 41, $arSet['ID'], Array(), Array( 'CODE' => 'ITEMS' ) );
		while ( $arPropValue = $dbPropValues->Fetch() )
		{
			if ( !$arPropValue['VALUE'] || $arPropValue['VALUE'] == $ElementID )
			{
				continue;
			}
			
			$arSetItemsIds[] = $arPropValue['VALUE'];
		}
		...
		{
			global $arSetFilter;
			$arSetFilter['ID'] = $arSetItemsIds;
			?>
				<h2>Комплект &laquo<?=$arSet['NAME']?>&raquo</h2>
...
<?php $APPLICATION->ShowViewContent('ITEMS_BLOCKS'); ?>
