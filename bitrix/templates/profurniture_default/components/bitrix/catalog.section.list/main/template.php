	...
	<ul class="row list-unstyled">
		...
		foreach ($arResult['SECTIONS'] as $key => $arSection):
			...
			if ($previousLevel && $arSection['DEPTH_LEVEL'] < $previousLevel)
				echo str_repeat('</ul></li>', ($previousLevel - $arSection["DEPTH_LEVEL"]) - 1);

			if (
				$arParams['SHOW_SHOW_MORE'] == 'Y' &&
				$index2 > $arParams['SHOW_COUNT_LVL2'] &&
				$moreShowed === false &&
				$index2 > 1
				)
			{
				?><li><a <?
					...
					?>href="<?=$arResult['SECTIONS'][$previousLevelOne]['SECTION_PAGE_URL']?>" <?
					?>title="" <?
				?>><?=Loc::getMessage('BUTTON.SHOW_MORE')?><?php
				if ($arParams['COUNT_ELEMENTS'] && intval($arResult['SECTIONS'][$previousLevelOne]['ELEMENT_CNT']) > 0):
					?> (<?=$arResult['SECTIONS'][$previousLevelOne]['COUNT_LVL2']?>)<?php
				endif;?>...</a></li><?php
				$moreShowed = true;
			}
