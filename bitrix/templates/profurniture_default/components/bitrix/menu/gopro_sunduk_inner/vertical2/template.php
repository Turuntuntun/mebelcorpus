...
if(is_array($arResult) && count($arResult)>0){
	?><div class="catalogmenu2_column fixed-left-menu"><?
		?><ul class="catalogmenu2 clearfix"><?
			?><li class="parent">
			
				<?/*?>
				<a  href="<?=$arParams['RSGOPRO_CATALOG_PATH']?>" class="parent"><?=GetMessage('RSGOPRO_CATALOG')?><?
			?><svg class="svg-icon menu"><use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#svg-menu"></use></svg>
				</a>
				<?*/?>
				
			<?
			?><ul style="top: 0;" class="first clearfix lvl1<?if($arParams['IS_MAIN']=='Y'):?> rs-show<?endif;?>"><?
			$previousLevel = 0;
			$index = 1;
					...
					if($arItem['DEPTH_LEVEL'] == 1){
						?><li class="first<?if($index>$max):?> more<?endif;?><?if($arItem['IS_LAST_LVL1']=='Y'):?> lastchild<?endif;?><?=(!empty($arItem['DETAIL_PICTURE']) ? ' catalogmenu2-icon' : '')?>"><?
							...
							/*?><ul <?
								...
								?><ul <?
								?>class="lvl<?if($arItem['DEPTH_LEVEL']>3):?>4<?else:?>1<?endif;?> <?
								?>lvl2-count-<?if($arItem['LVL2_COUNT']>3):?>4<?else:?><?=($arItem['LVL2_COUNT'])?><?endif;?> <?=( $arItem['SELECTED'] ? 'rs-show' : '' )?>"><?
						$index++;
					} else {
						if($arItem['DEPTH_LEVEL']==2){
							?><div class="mrow <?=( $arItem['SELECTED'] ? 'selected' : '' )?>"><?
						}
		...
		?><ul class="catalogmenusmall clearfix"><?
			?><li class="parent">
				<?/*?>
				<a href="<?=$arParams['RSGOPRO_CATALOG_PATH']?>" class="parent"><?=GetMessage('RSGOPRO_CATALOG')?><svg class="svg-icon menu"><use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#svg-menu"></use></svg></a>
				<?*/?>
			<?
