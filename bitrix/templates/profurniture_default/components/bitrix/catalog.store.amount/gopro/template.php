...
$isReplaceStoreString = false;
$isHideStoresString = true;
if (!empty($arParams['STORE_REPLACE_STRING']))
{
	$isReplaceStoreString = true;
}

if (empty($arParams['STORE_REPLACE_IF_NOT_AVAILABLE']))
{
	$arParams['STORE_REPLACE_IF_NOT_AVAILABLE'] == 'N';
}

if ($isReplaceStoreString)
{
	if (
		$arParams['STORE_REPLACE_IF_NOT_AVAILABLE'] == 'N' ||
		(
			$arParams['STORE_REPLACE_IF_NOT_AVAILABLE'] == 'Y' &&
			(int) $arParams['DATA_QUANTITY'][$arParams['FIRST_ELEMENT_ID']] <= 0
		)
	)
	{
		$isHideStoresString = false;
	}
}
...
?>
<?php if ($isReplaceStoreString): ?>
<div class="b-stores<?php if($arParams['STORE_REPLACE_IF_NOT_AVAILABLE'] == 'Y'): ?> js-stores-string<?php endif;?>"<?php if ($isHideStoresString): ?> style="display: none"<?php endif; ?>>
	<span><?=$arParams['STORE_REPLACE_STRING']?></span>
</div>
<?php endif; ?>

<div class="b-stores js-stores" data-firstElement="<?=$arParams['FIRST_ELEMENT_ID']?>" data-page="<?=$arParams['PAGE']?>" <?php if (!$isHideStoresString): ?> style="display: none"<?php endif; ?>>
