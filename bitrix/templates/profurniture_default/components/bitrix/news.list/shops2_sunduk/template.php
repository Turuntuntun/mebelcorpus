...
<?php if (is_array($arResult["ITEMS"]) && count($arResult["ITEMS"]) > 0): ?>
    <div class="shops2">
		...
		<?php if ($arParams['RSGOPRO_SHOW_TITLE'] == 'Y'): ?>
			<div class="nice-title">Наши магазины</div>
		<?php endif; ?>
                ...
                <div class="col-xs-12 col-sm-12 col-md-9 col-lg-9 js-shops">
                    <div class="shops2-panel__filters js-filter">
                        <?php foreach ($arResult['FILTER_TYPES'] as $arFilterType): ?>
                            <div class="btn-primary js-btn" data-filter="<?=htmlspecialcharsbx($arFilterType['XML_ID'])?>"><?=$arFilterType['VALUE']?></div>
                        <?php endforeach; ?>
                        <div class="btn-primary active js-btn"  data-filter=""><?=Loc::getMessage('SHOP_FILTER_ALL');?></div>
                    </div>
                </div>
            </div>
        </div>
