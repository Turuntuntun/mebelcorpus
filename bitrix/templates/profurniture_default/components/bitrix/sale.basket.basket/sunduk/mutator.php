...
foreach ($this->basketItems as $row)
{
	...
	$rowData['COLOR_SELECT'] = Array();
	
	$activeColorName = '';
	
	foreach ( $row['PROPS'] as $arProp )
	{
		if ( $arProp['CODE'] == 'ITEM_COLOR' )
		{
			$activeColorName = $arProp['VALUE'];
		}
	}
	
	$dbProps = CIBlockElement::GetProperty( 29, $row['PRODUCT_ID'], Array(), Array( 'CODE' => 'COLORS' ) );
	while ( $arProp = $dbProps->Fetch() )
	{
		$arColorData = $arColorsData[$arProp['VALUE']];
		$colorName = $arColorData['NAME'];
