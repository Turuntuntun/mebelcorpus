...
function RSGoPro_SetRecalcPrices($jsSet) {
	...
	// update insertdata in buy1click
	var arIds = [],
		arBuy1click = {};
	$jsSet.find('.items.js-line1').find('.js-element').each(function(){
		arIds.push($(this).data('elementid'));
	});
	arBuy1click = {RS_ORDER_IDS:arIds.join()};
	$jsSet.find('.js-buy1click').data('insertdata', arBuy1click)
