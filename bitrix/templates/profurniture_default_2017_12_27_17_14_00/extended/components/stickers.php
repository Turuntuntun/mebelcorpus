...
<div class="c-stickers"><?
    ...
    ?><?php foreach ($arParams['STICKERS_PROPS'] as $propCode):
        ...
        ?><?
        ?><?php if ($prop['VALUE'] != ''): ?><?
            ?><span class="c-stickers__sticker c-stickers__<?=$prop['VALUE']?>"<?php if ($prop['HINT'] != ''): ?>style="background-color: <?=$prop['HINT']?>;"<?php endif; ?>><?=$prop['NAME']?></span><?
        ?><?php endif; ?><?
    ?><?php endforeach; ?><?
    ...
    ?><?php if ($arParams['STICKERS_DISCOUNT_VALUE'] == 'Y'):
        ...
        ?><?
        ?><span class="c-stickers__sticker c-stickers__discount js-discount-value" <?php if ($val < 1): ?> style="display: none;"<?php endif; ?>>-<span class="js-discount-value"><?=$val?></span>%</span><?
    ?><?php endif; ?><?
