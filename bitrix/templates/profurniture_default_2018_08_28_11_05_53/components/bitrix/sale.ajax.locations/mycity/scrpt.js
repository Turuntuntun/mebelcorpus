...
function JsSuggestSale(oHandler, sParams, sParser, domain, ssubmit)
{
	...
	t.Show = function(result)
	{
		...
		TCJsUtils.show(t.oDiv, pos["left"], pos["bottom"], t.oObj);
...
var TCJsUtils =
{
	...
	show: function(oDiv, iLeft, iTop, oObj)
	{
		if (typeof oDiv != 'object')
			return;
		/*
		var pos = TCJsUtils.GetRealPos(oObj);
		if( $(oObj).parents('.fancybox-inner').length>0 )
		{
			iTop += 100;
		}
		*/
...
$(document).ready(function(){
	...
	$(document).on('focus','.mysityinp',function(){
		if( $(this).val()==$(this).data('old') ){
			$(this).val('');
		}
	}).on('blur','.mysityinp',function(){
		if( $(this).val()=='' && $(this).data('old')!='' ){
