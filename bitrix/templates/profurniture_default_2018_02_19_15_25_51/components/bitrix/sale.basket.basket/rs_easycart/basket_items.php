...
if($normalCount>0) {
				...
				if($arItem['DELAY']=='N' && $arItem['CAN_BUY']=='Y') {
					?><tr class="rsec_jsline" data-id="<?=$arItem['ID']?>"><?
						...
						?><td class="rsec_min rsec_cen"><?
							// checkbox
							?><input type="checkbox" name="DELETE_<?=$arItem['ID']?>" id="easycart_DELETE_<?=$arItem['ID']?>" value="Y"><?
							?><label for="easycart_DELETE_<?=$arItem['ID']?>"></label><?
						?></td><?
						?><td class="rsec_image rsec_min"><?
							...
							?></div><?
						?></td><?
						?><td class="rsec_nowrap rsec_product-name"><?
							// name
							if(strlen($arItem['DETAIL_PAGE_URL'])>0) {
								?><a href="<?=$arItem['DETAIL_PAGE_URL']?>"><?
							}
						...
						?><td class="rsec_nowrap rsec_min rsec_padd"><?
							?><span class="rsec_quantity"><?
								?><span class="rsec_quantity__inner"><?
									?><a class="rsec_minus"></a><?
									?><input type="text" class="rsec_quantity__input" name="QUANTITY_<?=$arItem['ID']?>" value="<?=$arItem['QUANTITY']?>" <?
						...
						?><td class="rsec_delete rsec_min rsec_cen rsec_padd"><?
							?><a class="rsec_delete" href="#easycart_DELETE_<?=$arItem["ID"]?>" title="<?=GetMessage('SALE_DELETE')?>"><svg class="svg-icon"><use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#svg-close-button"></use></svg></a><?
						?></td><?
					?></tr><?
				}
			}
	...
	?><div class="rsec_buttons rsec_clearfix"><?
		?><div class="rsec_leftp"><?
			if($arParams['HIDE_COUPON']!='Y') {
				?><div class="rsec_coupon rsec_totaltext"><?
					...
					?><input class="btn1 rsec_coup" type="button" name="BasketRefresh" value="<?=GetMessage('SALE_APPLY_BTN')?>" /><?
				?></div><?
			...
			?><input class="btn2 rsec_delall" type="button" name="BasketRefresh" value="<?=GetMessage('SALE_BTN_DEL_ALL')?>" /><?
			?><input class="btn2 rsec_delsome" type="button" name="BasketRefresh" value="<?=GetMessage('SALE_DELETE')?>" /><?
		?></div><?
		?><div class="rsec_rightp"><?
			...
			?><input class="btn1" type="submit" name="BasketRefresh" value="<?=GetMessage('SALE_ORDER')?>" onclick="location.href='<?=$arParams['PATH_TO_ORDER']?>';return false;" /><?
		?></div><?
	?></div><?
