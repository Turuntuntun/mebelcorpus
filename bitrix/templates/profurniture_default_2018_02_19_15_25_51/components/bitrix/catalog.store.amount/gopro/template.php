...
<?php $this->SetViewTarget('TABS_STOCKS'); ?>
<div class="b-stores-list">
	<div id="popupstores_<?=$arParams['~ELEMENT_ID']?>"><?
		?><?php foreach ($arResult['STORES'] as $key1 => $arStore): ?><?
			?><div class="b-stores-list__row js-stores__store<?=$arStore['ID']?>" style="display:<?=($arParams['SHOW_EMPTY_STORE'] == 'N' && $arResult['JS']['SKU'][$arParams['FIRST_ELEMENT_ID']][$arStore['ID']] <= 0 ? 'none' : '')?>;"><?
				$emptyVal = '';
