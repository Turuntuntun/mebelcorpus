	...
	if($arResult['ALLOW_ANONYMOUS']=='N' && !$USER->IsAuthorized())
	{
		...
		?><form action="<?=$arResult['FORM_ACTION']?>" method="post"><?
			...
			?><div class="line buttons clearfix"><?
				?><input class="btn3" type="submit" name="Save" value="<?=( $arResult['ID']>0?GetMessage('CT_BSE_BTN_EDIT_SUBSCRIPTION'):GetMessage('CT_BSE_BTN_ADD_SUBSCRIPTION'))?>" /><?
			?></div><?
			...
			if ($arResult['ID'] > 0 && $arResult['SUBSCRIPTION']['CONFIRMED'] <> 'Y') {
				?><div class="line border clearfix"><?
					...
					?><input name="CONFIRM_CODE" type="text" value="" placeholder="<?=GetMessage('CT_BSE_CONFIRMATION')?>" /><?
					?><input class="btn1" type="submit" name="confirm" value="<?=GetMessage('CT_BSE_BTN_CONF')?>" /><?
				?></div><?
			}
