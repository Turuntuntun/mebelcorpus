		...
		if (is_array($arResult['ITEMS']) && count($arResult['ITEMS']) > 0) {
						...
						?><tr class="rsec_jsline" data-id="<?=$arItem['ID']?>"><?
							...
							if ($ident != 'viewed') {
								?><td class="rsec_min rsec_cen"><?
									// checkbox
									?><input type="checkbox" name="DELETE_<?=$arItem['ID']?>" id="<?=$arParams['RS_TAB_IDENT']?>_DELETE_<?=$arItem['ID']?>" value="Y"><?
                                    ?><label for="<?=$arParams['RS_TAB_IDENT']?>_DELETE_<?=$arItem['ID']?>"></label><?
								?></td><?
							}
							?><td class="rsec_image rsec_cen rsec_min"><?
								...
								if (strlen($arItem['PREVIEW_PICTURE']['RESIZE']['src']) > 0) {
									?><img src="<?=$arItem['PREVIEW_PICTURE']['RESIZE']['src']?>" alt="<?=$strAlt?>" title="<?=$strTitle?>" /><?
								} else {
							...
							*/
							if ($ident!='viewed') {
								?><td class="rsec_min rsec_cen rsec_padd"><?
									?><a class="rsec_delete" href="#"><svg class="svg-icon"><use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#svg-close-button"></use></svg></a><?
								?></td><?
							}
						?></tr><?
					}
			...
			?><div class="rsec_buttons rsec_clearfix"><?
				if ($ident!='viewed') {
					?><div class="rsec_leftp"><?
						?><input class="btn2 rsec_delall" type="button" name="Refresh" value="<?=GetMessage('BTN_DEL_ALL')?>" /><?
						?><input class="btn2 rsec_delsome" type="button" name="Refresh" value="<?=GetMessage('DELETE')?>" /><?
					?></div><?
				}
				if ($ident == 'compare' && $arParams['COMPARE_RESULT_PATH'] != '') {
					?><div class="rsec_rightp"><?
						?><input class="btn1 rsec_delsome" type="button" name="Refresh" value="<?=GetMessage('TAB_NAME_GO_COMPARE')?>" onclick="location.href='<?=$arParams['COMPARE_RESULT_PATH']?>';return false;" /><?
					?></div><?
				}
			?></div><?
			...
			$this->SetViewTarget($arParams['RS_TAB_IDENT']);
				?><div class="rsec_orlink">
					<a class="<?=$tabJSIdent?> rsec_changer" href="#<?=$tabJSIdent?>"><?
						?><i class="rsec_iconka"></i><?
						?><span class="rsec_name"><?=GetMessage('TAB_NAME_'.$arParams['RS_TAB_IDENT'])?>&nbsp;</span><?
						?><span class="rsec_color rsec_cnt"><?=count($arResult['ITEMS'])?></span><?
					?></a><?
				?></div><?
			$this->EndViewTarget();
		} else {
			...
			$this->SetViewTarget($arParams['RS_TAB_IDENT']);
				?><div class="rsec_orlink"><?
					?><a class="<?=$tabJSIdent?> rsec_changer" href="#<?=$tabJSIdent?>"><?
						?><i class="rsec_iconka"></i><?
						?><span class="rsec_name"><?=GetMessage('TAB_NAME_'.$arParams['RS_TAB_IDENT'])?>&nbsp;</span><?
						?><span class="rsec_color rsec_cnt">0</span><?
					?></a><?
				?></div><?
			$this->EndViewTarget();
		}
