		...
		if ($arResult['SET_ITEMS']['PRICE'] != '') {
							...
							?><td class="val"><?
								?><div class="allprs"><?
									if ($arResult['SET_ITEMS']['OLD_PRICE']) {
										?> <span class="price old c-prices__value-old"><?=$arResult['SET_ITEMS']['OLD_PRICE']?></span><?
									}
			...
			?><div class="block buyset"><?
				?><noindex><form><?
					...
					$arBuy1click = array(
						'RS_ORDER_IDS' => implode(',', $arShowedIds)
					);
					?><a rel="nofollow" class="btn2 buy1click set nowrap fancyajax fancybox.ajax js-buy1click" href="<?=SITE_DIR?>include/popup/buy1click/" data-insertdata="<?=htmlspecialcharsbx(\Bitrix\Main\Web\Json::encode($arBuy1click))?>" title="<?=GetMessage('CATALOG_SET_BUY1CLICK')?>"><?=GetMessage('CATALOG_SET_BUY1CLICK')?></a><?
				?></form><noindex><?
			?></div><?
		} else {
			?><div class="block prs"><?
