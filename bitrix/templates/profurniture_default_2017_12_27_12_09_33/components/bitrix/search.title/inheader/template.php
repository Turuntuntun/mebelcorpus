	...
	<div id="<?=$CONTAINER_ID?>" class="searchinhead nowrap js-pseudo-focus-blur">
		<form class="js-form" action="<?=$arResult["FORM_ACTION"]?>" data-starthidden="<?=$arParams['START_HIDDEN']?>" data-showsearchbar="<?=$showSearchBar?>" data-window-width-jsopenser="<?=$arParams['WINDOW_WIDTH_MOBILE_OPENER_USE']?>">
			<div class="searchinhead__flex">
				...
				<div class="searchinhead__flexbox searchinhead__aroundtext js-pseudo-border-top js-pseudo-border-bottom"><input class="searchinhead__input js-pseudo-focus-blur-input" id="<?=$INPUT_ID?>" type="text" name="q" value="" size="40" maxlength="50" autocomplete="off" placeholder="<?=GetMessage("RSGOPRO_PLACEHOLDER")?>" /></div>
...
<?php
/* broke title search */
$CONTAINER_ID = $CONTAINER_ID.'broke';
$INPUT_ID = $INPUT_ID.'broke';
/* /broke title search */
?>
