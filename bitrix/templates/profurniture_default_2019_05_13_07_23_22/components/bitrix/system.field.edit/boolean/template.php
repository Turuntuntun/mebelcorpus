			...
			switch($arParams["arUserField"]["SETTINGS"]["DISPLAY"])
			{
				...
				default:
					?><span class="likeinput"><?
						...
						?><input type="checkbox" value="1" name="<?=$arParams["arUserField"]["FIELD_NAME"]?>" id="<?=$arParams["arUserField"]["FIELD_NAME"]?>" <?=($res ? "checked=\"checked\"": "")?>><?
						?><label for="<?=$arParams["arUserField"]["FIELD_NAME"]?>"></label><?
					?></span><?
				break;
			}
		?></div><?
